...
class ADSREnvelope:
    ...
    def get_ads_stepper(self):
        ...
        while True:
            l = len(steppers)
            if l > 0:
                # Keep yielding Attack value until it reaches full amplitude
                val = next(steppers[0])
                if l == 2 and val > 1:
                    # When Attack has finished, switch to yielding Decay
                    steppers.pop(0)
                    val = next(steppers[0])
                elif l == 1 and val < self.sustain_level:
                    # When Decay has finished, switch to yielding Sustain
                    steppers.pop(0)
                    val = self.sustain_level
            else:
                # No Attack and Decay scenario
                val = self.sustain_level
            yield val
